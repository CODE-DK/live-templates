...
@Component
@AllArgsConstructor
public class IbmMessageListener2 implements MessageListener {

    private final ObjectMapper objectMapper;

    @Override
    @JmsListener(
            selector = "listener = 'second'",
            destination = IBM_MESSAGES_QUEUE,
            containerFactory = CONTAINER_FACTORY
    )
    public void onMessage(Message message) {
        IbmMessage messageBody = readMessage(message);
        log.info("2) Receive new message from: {} with payload: {}", IBM_MESSAGES_QUEUE, messageBody);
    }
